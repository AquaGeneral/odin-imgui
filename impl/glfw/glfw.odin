...
setup_state :: proc(window: glfw.WindowHandle, install_callbacks: bool) {
    ...
    when ODIN_OS == .Windows {
        io.ime_WindowHandle = rawptr(glfw.GetWin32Window(state.window));
    }
...
update_mouse :: proc() {
    ...
    for i in 0..<len(io.mouse_down) {
        io.mouse_down[i] = state.mouse_just_pressed[i] || glfw.GetMouseButton(state.window, c.int(i)) != glfw.RELEASE;
    ...
    if io.config_flags & imgui.Config_Flags.NoMouseCursorChange != imgui.Config_Flags.NoMouseCursorChange {
